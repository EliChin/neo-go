...
// TrieStore is the MPT-based storage implementation for storing and retrieving
// blockchain data. TrieStore is supposed to be used within script invocations
// only, thus only storage-related operations are supported. All storage-related
// operations are being performed using historical storage data retrieved from
// MPT state. TrieStore is read-only and does not support put-related operations,
// thus, it should always be wrapped into MemCachedStore. TrieStore never changes
// the provided backed store.
type TrieStore struct {
	trie   *Trie
	backed storage.Store
}
...
func (m *TrieStore) Seek(rng storage.SeekRange, f func(k, v []byte) bool) {
	...
	process := func(pathToNode []byte, node Node, _ []byte) bool {
		if leaf, ok := node.(*LeafNode); ok {
			// (*Billet).traverse includes `from` path into result if so. It's OK for Seek, so shouldn't be filtered.
			kv := storage.KeyValue{
	...
	if err != nil && !errors.Is(err, errStop) {
		// TODO: maybe shouldn't panic here?
